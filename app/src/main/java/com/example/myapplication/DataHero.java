...
public class DataHero {
    ...
    static Collection<?extends ModelHero> getListDetail() {
        ArrayList<ModelHero> list = new ArrayList<>();

        for (int position = 0; position < heroNames.length; position++) {
            ModelHero hero = new ModelHero();
            hero.setHeroImage(heroesImages[position]);
            hero.setHeroName(heroNames[position]);
            hero.setHeroDesc(heroDetails[position]);
            list.add(hero);
        }
